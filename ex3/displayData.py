...
def display_data(data, img_width=20):
    ...
    cols_pixel = np.ceil(display_cols * img_width + (display_cols + 1) * pad)
    ...
    # the first pixel of every image is 1+(img_width+pad)*(n-1) or 1+(image_height+pad)*(n-1)
    for i in range(data.shape[0]):
        image_data = data[i, :].reshape((int(img_width), int(image_height)))
        row_index = np.floor(i / display_cols)
        cols_index = i % display_cols
        row_position = pad+(image_height+pad)*row_index
        cols_position = pad+(img_width+pad)*cols_index
        display_matrix[int(row_position):int(row_position+image_height), int(cols_position):int(cols_position+img_width)] = image_data[:, :]
